...
module.exports = {
  ...
  mode: 'development' // 默认为production, 可以手动设置为development(开发环境), 区别就是是否进行压缩混淆
}
```
...
```
指定打包的时的配置文件 `npx webpack --config webpack.custom.config.js`
...
2. 项目文件夹下直接创建index.html，与webpack.config.js平级，引入js文件 `<script src="/bundle.js"></script>`
引用dist下打包好的bundle.js文件并不会实时刷新，因为webpack实时打包好的文件放在/根目录下
要将index.html 放到src下可被直接访问的话 可设置 --contentBase src 

3. 运行：`npx webpack-dev-server`

4. 运行：`npx webpack-dev-server --contentBase src  --hot --open --port 8090`
--contentBase 设置根目录
--hot 开启热模块更替
--open 设置自动打开网页
--port 8090 设置端口
...
5. 配置`package.json`的scripts：`"dev": "webpack-dev-server --contentBase src --hot --open --port 8090"`
...
file-loader：加载相应文件
url-loader：对文件进行处理 limit表示如果图片大于5KB，就以路径形式展示，小于的话就用base64格式展示
...
在合适的时机调用对应��子的call方法即可，如需传入参数，可以在对应的钩子中定义好需要传入的参数，call时直接传入
